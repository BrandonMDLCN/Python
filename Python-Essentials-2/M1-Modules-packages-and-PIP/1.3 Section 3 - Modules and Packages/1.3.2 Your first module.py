...
# You will need two files to repeat these experiments. The first of them will be the module itself. It's empty now. 
# Don't worry, you're going to fill it with actual code soon.
...
# It doesn't matter that the module is empty for now. 
# The very first step has been done, but before you take the next step, we want you to take a look into the folder in which both files exist.
...
# You can look inside the file – the contents are completely unreadable to humans. It has to be like that, as the file is intended for Python's use only.
...
# As such, the file doesn't require lots of the checks needed for a pure source file, the execution starts faster, and it runs faster, too.

# Thanks to that, every subsequent import will go quicker than interpreting the source text from scratch.

# Python is able to check if the module's source file has been modified (in this case, the pyc file will be rebuilt) or not (in which case the pyc file may be run at once). 
...
# There's a cleverer way to utilize the variable, however. 
# If you write a module filled with a number of complex functions, you can use it to place a series of tests to check if the functions work properly.
...
""" # Step 7 """
# This module will contain two simple functions, and if you want to know how many times the functions have been invoked, 
# you need a counter initialized to zero when the module is being imported.
...
# the line starting with #! has many names - it may be called shabang, shebang, hashbang, poundbang or even hashpling (don't ask us why). 
    # The name itself means nothing here - its role is more important. From Python's point of view, it's just a comment as it starts with #. 
...
# To answer this question, we have to talk about how Python searches for modules. 
# There's a special variable (actually a list) storing all locations (folders/directories) that are searched in order to find a module 
# which has been requested by the import instruction.
