...
# As you can see, there are two columns in the list, one showing the name of the installed package, and the other showing the version of the package. 
# We can’t predict the state of your Python installation.

# The only thing we know for sure is that your list contains the two lines we see on our list: pip and setuptools. 
# This happens because the OS is convinced that a user wanting pip will very likely need the setuptools soon. It’s not wrong.

# The pip list isn't very informative, and it may happen that it won't satisfy your curiosity. 
# Fortunately, there’s a command that can tell you more about any of the installed packages (note the word installed). The syntax of the command looks as follows:
...
# You may ask where this data comes from? Is pip really so perceptive? Not at all – the information appearing on the screen is taken from inside the package being shown. 
# In other words, the package's creator is obliged to equip it with all the needed data (or to express it more precisely – metadata).
...
# The power of pip comes from the fact that it’s actually a gateway to the Python software universe. 
# Thanks to that, you can browse and install any of the hundreds of ready-to-use packages gathered in the PyPI repositories. 
...
# This means that you have to have a network connection working whenever you’re going to ask pip for anything that may involve direct interactions with 
...
# Two possible scenarios may be put into action now:
    ...
    # you want to install a new package for you only – it won't be available for any other user (account) existing on your computer; 
        # this procedure is the only one available if you can’t elevate your permissions and act as a system administrator;
...
# Pip has a habit of displaying fancy textual animation indicating the installation progress, so watch the screen carefully – don't miss the show! 
# If the process is successful, you’ll see something like this:
