...
# Sometimes, it may be necessary to find out information unrelated to Python. 
# For example, you may need to know the location of your program within the greater environment of the computer.
...
# The layers are:
    ...
    # the next layer of the pyramid is filled with the OS (operating system) 
    ...
    # the bottom-most layer is hardware - the processor (or processors), network interfaces, human interface devices (mice, keyboards, etc.) 
        # and all other machinery needed to make the computer run; the OS knows how to drive it, and uses lots of tricks to conduct all parts in a consistent rhythm.
...
# This means than some of your (or rather your program's) actions have to travel a long way to be successfully performed - imagine that:
    ...
    # Python accepts the order, rearranges it to meet local OS requirements (it's like putting the stamp "approved" on your request) 
        # and sends it down (this may remind you of a chain of command)
    # The OS checks if the request is reasonable and valid (e.g., whether the file name conforms to some syntax rules) and tries to create the file; 
        # such an operation, seemingly very simple, isn't atomic - it consists of many minor steps taken by...
...
# thus, its output is rather addressed to humans than to automated processing (you'll see it soon).
...
# Sometimes, you may just want to know the generic name of the processor which runs your OS together with Python and your code 
# - a function named machine() will tell you that. As previously, the function returns a string.
